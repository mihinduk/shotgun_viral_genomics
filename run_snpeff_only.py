#!/usr/bin/env python3
"""
Run just the snpEff annotation portion of the viral pipeline.

This script is useful for re-annotating existing VCF files with the improved
GFF3 approach for complex viral genomes with polyproteins and mature peptides.

Example usage:
    python3 run_snpeff_only.py --vcf sample_vars.filt.vcf --accession GQ433359.1 --snpeff-jar /path/to/snpEff.jar --add-to-snpeff
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run just the snpEff annotation portion of the viral pipeline",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    
    # Input/output options
    parser.add_argument("--vcf", required=True, help="Input VCF file (filtered)")
    parser.add_argument("--output-dir", help="Output directory (defaults to same as VCF)")
    
    # Reference genome options
    parser.add_argument("--accession", required=True, help="Reference genome accession number (e.g., GQ433359.1)")
    parser.add_argument("--reference", help="Path to local reference genome (when downloaded manually)")
    
    # SnpEff options
    parser.add_argument("--add-to-snpeff", action="store_true", help="Attempt to add genome to snpEff if not present")
    ...
    parser.add_argument("--parse-script", help="Path to parse_snpEff_annotated_vcf_for_collaborators.pl script")
    parser.add_argument("--skip-parse", action="store_true", help="Skip parsing the annotation")
    parser.add_argument("--min-depth", type=int, default=200, help="Minimum read depth for filtering in parsing step")
...
def generate_custom_gff3(accession: str, gb_path: str, output_dir: str) -> str:
    ...
    if not os.path.exists(converter_script):
        ...
        potential_paths = [
            ...
            "../convert_gb_to_gff_advanced.py",
            "/Users/handley_lab/Handley Lab Dropbox/virome/KM_algorithm_dev/shotgun_viral_genomics/convert_gb_to_gff_advanced.py"
        ]
...
def download_reference_genome(accession: str, output_dir: str) -> str:
    ...
    Args:
        accession: The genome accession number (e.g., GQ433359.1)
...
def fix_vcf_for_snpeff(vcf_path: str) -> str:
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    fix_script = os.path.join(script_dir, "fix_vcf_for_snpeff.py")
...
def annotate_variants(vcf_file: str, accession: str, snpeff_jar: str, java_path: str = "java", parse_script_path: Optional[str] = None, min_depth: int = 200, skip_parse: bool = False) -> Dict[str, str]:
    """
    ...
    Args:
        ...
        snpeff_jar: Path to snpEff.jar
        java_path: Path to Java executable
        parse_script_path: Path to parsing Perl script
    ...
    if not skip_parse and os.path.exists(ann_tsv) and os.path.getsize(ann_tsv) > 0:
        ...
        script_found = False
        if parse_script_path and os.path.exists(parse_script_path):
            script_found = True
            parse_script = parse_script_path
        else:
            # Try to find it in the same directory
            potential_paths = [
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "parse_snpEff_annotated_vcf_for_collaborators.pl"),
                "./parse_snpEff_annotated_vcf_for_collaborators.pl",
                "/Users/handley_lab/Handley Lab Dropbox/virome/KM_algorithm_dev/shotgun_viral_genomics/parse_snpEff_annotated_vcf_for_collaborators.pl"
            ]
            ...
            for path in potential_paths:
                if os.path.exists(path):
                    script_found = True
                    parse_script = path
                    logger.info(f"Found parse script at: {parse_script}")
                    break
...
def main():
    ...
    try:
        ...
        # Step 1: Download reference genome if needed
        if args.reference:
            ref_path = args.reference
            logger.info(f"Using provided reference genome: {ref_path}")
        else:
            ref_path = download_reference_genome(args.accession, output_dir)
        
        # Step 2: Check if genome is in snpEff database
        in_database = check_snpeff_database(args.accession, args.snpeff_jar, args.java_path)
        
        # Step 3: Add genome to snpEff database if needed
        if not in_database:
            logger.info(f"Genome {args.accession} not found in snpEff database")
            if args.add_to_snpeff:
                logger.info(f"Adding genome {args.accession} to snpEff database")
                success = add_genome_to_snpeff(args.accession, ref_path, args.snpeff_jar, args.java_path)
                if not success:
                    raise RuntimeError(f"Failed to add genome {args.accession} to snpEff database")
            else:
                raise RuntimeError(f"Genome {args.accession} not in snpEff database. Use --add-to-snpeff to add it.")
        
        # Step 4: Run snpEff annotation
